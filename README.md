...
Веб-демонстрации выполняются полностью в браузере. На странице одновременно открыт один пример; остальные выбираются по ссылкам в верхнем меню. После лабораторной №4 отдельной пятой ссылкой открывается итоговый проект на любую тему по RL. Для каждого примера приведены постановка задачи, описание данных, анимация, прогноз до запуска, динамический вывод и расширенный итоговый тест из семи вопросов.

В каждом стенде есть учебная песочница «свой код ↔ код ИИ». Студент вручную дописывает политику или ключевой модуль алгоритма; вставка и перенос готового кода отключены. Локальный анализатор проверяет обязательные конструкции и риски утечки, после чего можно открыть демонстрационный вариант ИИ, прочитать объяснение и сравнить решения. Структурная оценка не заменяет запуск Python, unit-тесты и эксперимент на данных.
