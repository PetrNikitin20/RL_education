...
Веб-демонстрации выполняются полностью в браузере. На странице одновременно открыт один пример; остальные выбираются по ссылкам в верхнем меню. Для каждого примера приведены постановка задачи, описание данных, анимация, прогноз до запуска, динамический вывод, входная диагностика, итоговый тест и экспорт протокола. Режим преподавателя показывает временное разбиение и методические комментарии.
